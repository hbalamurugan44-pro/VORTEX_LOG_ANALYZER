...
def expand_zip_files(files):
    """Given a mix of plain log/txt uploads and .zip uploads, return a flat list of
    log/txt-like file objects, pulling every .log/.txt entry out of any zip(s)."""
    expanded = []
    for f in files:
                ...
                with zipfile.ZipFile(BytesIO(raw)) as zf:
                    for info in zf.infolist():
                        ...
                        if lower.endswith(".log") or lower.endswith(".txt"):
                            data = zf.read(info)
                            # use just the base filename, ignoring any folder path inside the zip
                            base_name = os.path.basename(info.filename) or info.filename
                            expanded.append(_ExtractedFile(base_name, data))
            except zipfile.BadZipFile:
                st.error(f"'{f.name}' doesn't look like a valid zip file — skipping it.")
        else:
            expanded.append(f)
    return expanded
